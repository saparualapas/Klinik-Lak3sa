...
// Setup page: simpan URL ke SPREADSHEET dulu, lalu localStorage
async function submitSetup() {
  ...
  if (btn) { btn._l = btn.innerHTML; btn.innerHTML = '<span class="sp2"></span> Menyimpan...'; btn.disabled = true; }
  
  API_URL = url;
  toast('Menghubungi server...', 'in');
  
  try {
    // Simpan ke spreadsheet dulu via setupUrl action
    const r = await api({ action: 'setupUrl', url }, 'POST');
    if (r.success) {
      // Simpan ke localStorage setelah berhasil tersimpan di spreadsheet
      localStorage.setItem('klinik_api', url);
      if (r.activeSheet) {
        activeSheet = r.activeSheet;
        updSheetBadge();
        // Simpan active_sheet ke localStorage juga sebagai cache
        localStorage.setItem('klinik_sheet', r.activeSheet);
      }
      toast('✅ URL & konfigurasi berhasil disimpan!', 'ok');
      setTimeout(() => showLogin(), 900);
    } else {
      toast('Gagal menyimpan ke spreadsheet: ' + (r.message || ''), 'er');
    }
  } catch(e) {
    // Fallback: simpan hanya ke localStorage jika server gagal
    localStorage.setItem('klinik_api', url);
    toast('⚠️ Tersimpan lokal saja. Cek URL & coba lagi.', 'er');
    setTimeout(async () => {
      await loadConfig();
      showLogin();
    }, 1200);
  ...
  if (btn) { btn.innerHTML = btn._l || 'Simpan & Lanjutkan →'; btn.disabled = false; }
}
...
async function loadConfig() {
  try {
    ...
    if (r.activeSheet) {
      ...
      if (cached && !activeSheet) {
        activeSheet = cached;
        updSheetBadge();
      }
    }
    // Sinkronkan API URL dari spreadsheet jika ada
    if (r.apiUrl) {
      API_URL = r.apiUrl;
    ...
    const cachedSheet = localStorage.getItem('klinik_sheet');
    if (cachedSheet && !activeSheet) {
...
async function saveConfigToSheet(key, value) {
  try {
    await api({ action: 'saveConfig', [key]: value }, 'POST');
...
async function initDash() {
  // Reload config dari spreadsheet untuk pastikan activeSheet terkini
  await loadConfig();
  await loadSheets();
  loadStats();
}
...
async function doSave() {
  ...
  setBtn('saveBtn', true);
  showCrudSplash('💾 Menyimpan data pemeriksaan...');
...
async function updVisit() {
  ...
  showCrudSplash('✏️ Memperbarui data kunjungan...');
  const r=await api(payload,'POST');
  hideCrudSplash();
...
async function delVisit(i) {
  ...
  if (!activeSheet) { toast('Sheet tidak aktif','er'); return; }
  showCrudSplash('🗑️ Menghapus data kunjungan...');
...
async function aktivasiSheet() {
  ...
  updSheetBadge();
  // Simpan ke spreadsheet DAN localStorage
...
async function addUser(){
  ...
  if(!u||!p){toast('Isi username dan password!','er');return;}
  showCrudSplash('👤 Menambahkan user "'+u+'"...');
...
async function delUser(u){
  ...
  showCrudSplash('🗑️ Menghapus user "'+u+'"...');
